...
namespace Server.Common.Data
{
    ...
    public static class Database
    {
        ...
        public static int Count(string table, string field, string constraints, params object[] args)
        {
            using (MySqlDataReader reader = MySqlHelper.ExecuteReader(Database.ConnectionString,
                string.Format("SELECT COUNT({1}) FROM {0} WHERE {2}", table, field, string.Format(constraints, args))))
            {
                if (reader.Read())
                {
