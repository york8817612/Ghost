...
namespace Server.Ghost.Characters
{
    public class CharacterKeyMap : Dictionary<string, Shortcut>
    {
        ...
        public void Load()
        {
            ...
            {
                this.Add((string)datum.quickslot, new Shortcut(datum.skillID, (byte)datum.type, (byte)datum.slot));
            }
        }

        public KeyValuePair<string, Shortcut> Skill(int Type, int Slot)
        {
            foreach (KeyValuePair<string, Shortcut> shortcut in this)
            {
                if (shortcut.Value.Type == Type && shortcut.Value.Slot == Slot)
                    return shortcut;
            }
            return new KeyValuePair<string, Shortcut>("", null);
        }
